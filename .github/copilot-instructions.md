...
2. Run `golangci-lint run --fix` to identify linting issues
3. Target one issue at a time and fix them manually
    - Use `golangci-lint run --enable-only=<linter_name>` to focus on a specific linter if needed
4. Run `go test ./...` again to ensure no tests are broken after fixes
5. Repeat steps 2-6 until `golangci-lint run` shows no issues
